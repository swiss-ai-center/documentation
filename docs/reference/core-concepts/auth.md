...
sequenceDiagram
    ...
    Note over User, Core AI Engine: Given an auth token, the user can access the engine
    User->>Core AI Engine: Start service (Auth token + service parameters)
    Core AI Engine->>User: Start service (Auth token + service parameters)
```
